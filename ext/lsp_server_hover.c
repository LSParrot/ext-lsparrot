...
static inline zend_string *lsp_hover_builtin_constant_type(zend_string *word)
{
	...
	value = zend_get_constant(name);
	zend_string_release(name);
	if (!value) {
		return NULL;
	}

	return lsp_hover_zval_type(value);
}
...
static inline bool lsp_hover_offset_in_string_literal(lsp_document *document, size_t offset)
{
	zend_long id;
	...
	for (i = 0; i < count; i++) {
		...
		token_offset = (size_t) lsp_token_long(token, "offset", 0);
		token_length = (size_t) lsp_token_long(token, "length", 0);
...
/* Hover for a property declaration ("private string $secret = ...;"):
 * surface the declared type instead of the generic "variable" detail. */
static inline zend_string *lsp_hover_property_declaration_markdown(lsp_document *document, zend_string *word, size_t offset)
{
	zend_long body_depth = 0;
	zend_string *text_value, *detail, *markdown;
	zval *tokens_zv, *token;
	HashTable *tokens;
	uint32_t i, count;
	size_t class_start = 0, body_start = 0, body_end = 0, token_offset, token_length;
	...
	for (i = 0; i < count; i++) {
		...
		text_value = lsp_token_string(token, "text");
		if (!text_value || !zend_string_equals(text_value, word)) {
			return NULL;
		}
		...
		detail = lsp_property_completion_detail(document->text, token, false);
		if (!detail) {
			return NULL;
		}
...
{
	zend_long body_depth = 0;
	zend_string *text_value, *property_name, *detail;
	...
	property_name = strpprintf(0, "$%s", ZSTR_VAL(member));
	tokens = Z_ARRVAL_P(tokens_zv);
	count = zend_hash_num_elements(tokens);
	detail = NULL;
	...
	for (i = 0; i < count && !detail; i++) {
		...
		if (lsp_token_name_equals(token, "T_VARIABLE")) {
			text_value = lsp_token_string(token, "text");
			if (text_value && zend_string_equals(text_value, property_name) &&
		...
		text_value = lsp_token_string(name_token, "text");
		if (text_value && zend_string_equals(text_value, member)) {
...
/* Hover for "$receiver->member" / "$this->member": resolve the receiver
 * class and surface the member declaration (type or signature) from the
 * class member cache. */
static inline zend_string *lsp_hover_member_access_markdown(lsp_server *server, lsp_document *document, zend_string *word, size_t offset)
{
	...
	arrow_end = word_start;
	while (arrow_end > 0 && isspace((unsigned char) value[arrow_end - 1])) {
		arrow_end--;
	}

	if (arrow_end < 2 || value[arrow_end - 1] != '>' || value[arrow_end - 2] != '-') {
		return NULL;
	}
	...
	if (!detail) {
		/* The member cache only holds public/protected members. "$this->"
		 * receivers may legally access private members, so scan the
		 * enclosing class declaration directly. */
		receiver_end = arrow_end >= 2 ? arrow_end - 2 : 0;
		if (receiver_end >= sizeof("$this") - 1 &&
			memcmp(value + receiver_end - (sizeof("$this") - 1), "$this", sizeof("$this") - 1) == 0
		) {
			detail = lsp_hover_own_member_detail(document, word, offset);
		}
	}
...
/* ------------------------------------------------------------------------
 * Rich symbol hover: resolve the hovered word through the project symbol
 * index and render "FQCN: declaration header" (e.g. "Foo\\Bar: abstract
 * class Bar implements Baz"), reading the real declaration from the
 * defining file. Covers class-likes, functions, and constants, including
 * use sites in other files.
 * ------------------------------------------------------------------------ */

static inline zend_string *lsp_hover_collapse_whitespace(const char *value, size_t length)
{
	smart_str collapsed = {0};
	size_t i;
	bool in_space = false;

	while (length > 0 && isspace((unsigned char) value[0])) {
		value++;
		length--;
	}
	...
	for (i = 0; i < length; i++) {
		...
		if (in_space && collapsed.s) {
			smart_str_appendc(&collapsed, ' ');
		}
		in_space = false;
		smart_str_appendc(&collapsed, value[i]);
	}

	smart_str_0(&collapsed);

	return collapsed.s ? collapsed.s : zend_string_init("", 0, 0);
}

/* Extend a class-like header start backwards over declaration modifiers
 * (abstract / final / readonly) so they appear in the hover. */
static inline size_t lsp_hover_header_start_with_modifiers(zend_string *text, size_t class_start)
{
	static const char *modifiers[] = { "abstract", "final", "readonly" };
	const char *value = ZSTR_VAL(text);
	size_t start = class_start, word_start, word_length, i;
	bool matched;

	for (;;) {
		word_start = start;
		while (word_start > 0 && isspace((unsigned char) value[word_start - 1])) {
			word_start--;
		}

		word_length = 0;
		while (word_start > word_length && lsp_doc_is_identifier_char(value[word_start - word_length - 1])) {
			word_length++;
		}
		...
		for (i = 0; i < sizeof(modifiers) / sizeof(modifiers[0]); i++) {
			if (word_length == strlen(modifiers[i]) &&
				strncasecmp(value + word_start - word_length, modifiers[i], word_length) == 0
			) {
				matched = true;
				break;
			}
		...
		start = word_start - word_length;
	}
}

static inline zend_string *lsp_hover_symbol_file_contents(lsp_server *server, zend_string *path, bool *owns_contents)
{
	...
	*owns_contents = true;
	...
	return lsp_read_file(path);
}

static inline zend_string *lsp_hover_class_declaration_header(zend_string *contents, zend_string *fqcn)
{
	zend_long body_depth = 0;
	zend_string *header;
	size_t class_start = 0, body_start = 0, body_end = 0, header_start, header_end;

	/* lsp_find_class_header_for_name compares against the RESOLVED declared
	 * name, so it expects the fully qualified class name. */
	if (!lsp_find_class_header_for_name(contents, fqcn, &class_start, &body_start, &body_end, &body_depth)) {
		return NULL;
	}

	header_start = lsp_hover_header_start_with_modifiers(contents, class_start);
	header_end = body_start;
	while (header_end > header_start && (ZSTR_VAL(contents)[header_end - 1] == '{' || isspace((unsigned char) ZSTR_VAL(contents)[header_end - 1]))) {
		header_end--;
	}
	...
	header = lsp_hover_collapse_whitespace(ZSTR_VAL(contents) + header_start, header_end - header_start);
...
static inline zend_string *lsp_hover_function_declaration_header(zend_string *contents, const char *short_name, size_t short_length)
{
	zend_string *label, *detail;
	zval tokens_zv, *token, *name_token;
	HashTable *tokens;
	...
	for (i = 0; i < count; i++) {
		...
		if (!label || ZSTR_LEN(label) != short_length || strncasecmp(ZSTR_VAL(label), short_name, short_length) != 0) {
			continue;
		}

		detail = lsp_function_signature_detail(contents, name_token, tokens, name_index, 0, NULL);
		if (detail) {
			label = strpprintf(0, "function %s", ZSTR_VAL(detail));
			zend_string_release(detail);
			detail = label;
		}
		break;
...
static inline const char *lsp_hover_symbol_kind_word(char kind)
{
	switch (kind) {
...
static inline zend_string *lsp_hover_symbol_markdown_for(lsp_server *server, zend_string *fqcn, char expected_kind)
{
	const char *path_value, *short_name, *stored_value;
	zend_string *path, *contents, *header, *markdown, *stored;
	size_t path_length = 0, short_length;
	char found_kind = '\0';
	bool owns_contents = false;

	stored_value = NULL;
	path_value = lsp_symbol_index_lookup(&server->symbol_index, expected_kind, fqcn, &found_kind, &stored_value, &path_length);
	if (!path_value || path_length == 0) {
		return NULL;
	}

	/* Display the symbol with the casing it was declared with, regardless of
	 * how the hovered reference happens to be spelled (lookups are
	 * case-insensitive). */
	stored = stored_value ? zend_string_init(stored_value, strlen(stored_value), 0) : zend_string_copy(fqcn);
	short_name = lsp_basename_from_fqcn(ZSTR_VAL(stored), ZSTR_LEN(stored), &short_length);
	path = zend_string_init(path_value, path_length, 0);
	contents = lsp_hover_symbol_file_contents(server, path, &owns_contents);
	header = NULL;

	if (contents != zend_empty_string) {
		if (found_kind == LSP_SYMBOL_FUNCTION) {
			header = lsp_hover_function_declaration_header(contents, short_name, short_length);
		} else if (found_kind != LSP_SYMBOL_CONSTANT) {
			header = lsp_hover_class_declaration_header(contents, stored);
		}
	}

	if (!header) {
		header = strpprintf(0, "%s %.*s", lsp_hover_symbol_kind_word(found_kind), (int) short_length, short_name);
	}

	markdown = strpprintf(0, "`%s: %s`\n\nLSParrot Engine", ZSTR_VAL(stored), ZSTR_VAL(header));
...
static inline zend_string *lsp_hover_project_symbol_markdown(lsp_server *server, lsp_document *document, zend_string *word)
{
	...
	if (ZSTR_LEN(word) == 0 || ZSTR_VAL(word)[0] == '$' || !lsp_doc_is_identifier_start(ZSTR_VAL(word)[ZSTR_VAL(word)[0] == '\\' ? 1 : 0])) {
		return NULL;
	}
	...
	markdown = NULL;
	if (resolved) {
		markdown = lsp_hover_symbol_markdown_for(server, resolved, LSP_SYMBOL_CLASS);
		if (!markdown) {
			markdown = lsp_hover_symbol_markdown_for(server, resolved, LSP_SYMBOL_FUNCTION);
		}
		if (!markdown) {
			markdown = lsp_hover_symbol_markdown_for(server, resolved, LSP_SYMBOL_CONSTANT);
		}
		zend_string_release(resolved);
	}

	if (!markdown) {
		/* Unqualified functions and constants fall back to the global
		 * namespace in PHP; probe the raw word as well. */
		markdown = lsp_hover_symbol_markdown_for(server, word, LSP_SYMBOL_FUNCTION);
	}
	if (!markdown) {
		markdown = lsp_hover_symbol_markdown_for(server, word, LSP_SYMBOL_CONSTANT);
	}
...
extern void lsp_lsparrot_hover(lsp_server *server, zval *return_value, lsp_document *document, zval *position)
{
	...
	/* No symbol hover inside string literals: words in literal content must
	 * not resolve as class or function references. */
	if (lsp_hover_offset_in_string_literal(document, offset)) {
