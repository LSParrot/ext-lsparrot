#ifndef LSP_PHP81_ABSTRACTION_H
# define LSP_PHP81_ABSTRACTION_H

# include "lsp_internal.h"

static inline const char *lsp_php81_ast_kind_name(zend_ast_kind kind)
{
	switch (kind) {
...
static inline bool lsp_php81_ast_is_decl(zend_ast *ast)
{
	return zend_ast_is_special(ast) && ast->kind >= ZEND_AST_FUNC_DECL;
}

static inline bool lsp_php81_ast_is_opaque_node(zend_ast_kind kind)
{
	return kind == ZEND_AST_ZNODE;
}

static inline bool lsp_php81_string_equals_cstr(zend_string *value, const char *literal, size_t length)
{
	return ZSTR_LEN(value) == length && memcmp(ZSTR_VAL(value), literal, length) == 0;
}
