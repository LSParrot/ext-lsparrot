...
static inline char lsp_class_like_symbol_kind_for_file(zend_string *path, zend_string *fqcn)
{
	...
	uint32_t i, count;
	size_t label_length;
	char fallback = LSP_SYMBOL_CLASS;
	...
	for (i = 0; i < count; i++) {
		...
		if (ZSTR_LEN(label) == label_length && strncasecmp(ZSTR_VAL(label), label_value, label_length) == 0) {
			fallback = lsp_class_like_symbol_kind_from_token(token);
			break;
		}
	}
...
static inline bool lsp_token_is_namespace_name(zval *token)
{
...
static inline zend_string *lsp_namespace_from_tokens(HashTable *tokens, uint32_t start)
{
	zend_string *text;
	zval *token;
	...
	for (i = start; i < count; i++) {
		...
		if (!lsp_token_is_namespace_name(token)) {
			continue;
		}
...
static inline void lsp_index_declared_symbols_in_file(lsp_server *server, zend_string *path)
{
	zend_long depth = 0, class_depth = -1;
	zend_string *contents, *namespace_name, *name;
	zval tokens_zv, *token;
	HashTable *tokens;
	uint32_t i, count;
	bool pending_class_like = false;
	...
	for (i = 0; i < count; i++) {
		...
		if (lsp_token_name_equals(token, "T_NAMESPACE")) {
			...
			namespace_name = lsp_namespace_from_tokens(tokens, i + 1);
