...
/* Flush gcov counters before _exit(2) in forked workers so coverage runs
 * observe code executed in child processes. LSPARROT_COVERAGE is defined by
 * the pskel coverage build; everywhere else this is a no-op. A strong
 * reference is required because linkers do not extract libgcov.a members
 * for weak references. */
#if defined(LSPARROT_COVERAGE)
extern void __gcov_dump(void);
#endif

static inline void lsp_coverage_flush(void)
{
#if defined(LSPARROT_COVERAGE)
	__gcov_dump();
#endif
