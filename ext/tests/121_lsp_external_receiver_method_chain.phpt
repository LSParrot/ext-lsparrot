--TEST--
LSP completes method chains on external receivers and self-returning methods
--EXTENSIONS--
lsparrot
--FILE--
<?php
$root = '/tmp/lsp-external-chain-test';
...
file_put_contents($root . '/src/Builder.php', <<<'PHP'
<?php
namespace ChainFix;
...
{
    public function viaParameter(Builder $builder): int
    {
        return $builder->withSelf()->ext
    }
}
PHP;
...
{
    public function viaNew(): int
    {
        $b = new Builder();
        return $b->withStatic()->ext
    }
}
PHP;

file_put_contents($root . '/src/Consumer.php', $demo);
file_put_contents($root . '/src/OtherConsumer.php', $demoNew);
file_put_contents($root . '/vendor/composer/autoload_classmap.php', "<?php\nreturn [\n" .
    "    'ChainFix\\\\Builder' => " . var_export($root . '/src/Builder.php', true) . ",\n" .
    "    'ChainFix\\\\Consumer' => " . var_export($root . '/src/Consumer.php', true) . ",\n" .
    "    'ChainFix\\\\OtherConsumer' => " . var_export($root . '/src/OtherConsumer.php', true) . ",\n" .
    "];\n");
...
$uri = 'file://' . $root . '/src/Consumer.php';
$uriNew = 'file://' . $root . '/src/OtherConsumer.php';
$messages = [
    ...
    ['jsonrpc' => '2.0', 'method' => 'textDocument/didOpen', 'params' => ['textDocument' => ['uri' => $uriNew, 'languageId' => 'php', 'version' => 1, 'text' => $demoNew]]],
    // Chain rooted at a parameter-typed receiver through a self-returning method.
    ['jsonrpc' => '2.0', 'id' => 2, 'method' => 'textDocument/completion', 'params' => ['textDocument' => ['uri' => $uri], 'position' => lsp_position_after($demo, '$builder->withSelf()->ext')]],
    // Chain rooted at a `new` assignment through a static-returning method.
    ['jsonrpc' => '2.0', 'id' => 3, 'method' => 'textDocument/completion', 'params' => ['textDocument' => ['uri' => $uriNew], 'position' => lsp_position_after($demoNew, '$b->withStatic()->ext')]],
...
$viaParameter = lsp_item(lsp_response($decoded, 2), 'extra');
$viaNew = lsp_item(lsp_response($decoded, 3), 'extra');

if ($code !== 0) {
    echo "FAILED: process exit\n";
    var_dump($code, $stderr);
} elseif (!$viaParameter) {
    echo "FAILED: parameter-typed receiver chain did not complete extra()\n";
    var_dump(lsp_response($decoded, 2));
} elseif (!$viaNew) {
    echo "FAILED: new-assignment receiver chain did not complete extra()\n";
    var_dump(lsp_response($decoded, 3));
} else {
    echo "OK\n";
}
